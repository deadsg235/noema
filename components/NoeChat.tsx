...
export default function NoeChat({ state, onStateUpdate }: Props) {
  ...
  async function send() {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: "noe", text: data.reply, cluster: data.state.cluster }])
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="flex flex-col h-full">
        ...
        <AnimatePresence initial={false}>
          {messages.map((msg, i) => (
            ...
            >
              {msg.role === "noe" ? (
                <div
                  className="max-w-[85%] px-3 py-2 font-mono text-sm leading-relaxed text-white/70"
                ...
                <div className="max-w-[85%] px-3 py-2 rounded-xl bg-white/8 font-mono text-sm text-white/70 border border-white/5">
                  {msg.text}
                </div>
              )}
            </motion.div>
      ...
      <div className="p-4 border-t border-white/5">
        <div className="flex gap-2">
          <input
            className="flex-1 bg-white/[0.03] border border-white/8 rounded-lg px-3 py-2 font-mono text-sm text-white/80 placeholder-white/15 outline-none focus:border-white/20 transition-colors"
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg font-mono text-xs uppercase tracking-widest transition-all disabled:opacity-20 font-bold"
